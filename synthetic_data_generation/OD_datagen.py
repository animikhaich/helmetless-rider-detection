...
all_background_paths = glob.glob(os.path.join(bg_root_path, '*'))

if len(all_background_paths) < 1:
...
all_objects_paths = glob.glob(os.path.join(objects_root_paths, '*', '*'))

if len(all_objects_paths) < 1:
...
min_objects = 3
max_objects = 7
output_image_path = os.path.join('outputs', 'images')
output_annot_path = os.path.join('outputs', 'annotations')
image_counter = 0

if not os.path.isdir(output_image_path): os.makedirs(output_image_path)
if not os.path.isdir(output_annot_path): os.makedirs(output_annot_path)

for combinations in tqdm.tqdm(range(total_combinations)):
    bg_image_path = np.random.choice(all_background_paths, 1)[0]
    object_paths = np.random.choice(all_objects_paths, np.random.randint(min_objects, max_objects+1))
    ...
    for object_path in object_paths:
        ...
        if object_h < object_w:
            target_height = round(bg_h * object_to_bg_width_ratio)
            object_image = imutils.resize(object_image, height=target_height)
        else:
            target_width = round(bg_w * object_to_bg_width_ratio)
    ...
    bg_image.save(os.path.join(output_image_path, f"{image_counter}.jpg"))
    filename.text = f"{image_counter}.jpg"

    # Write the XML File
    xml_str = tostring(top).decode()
    etree_xml = etree.fromstring(xml_str)
    with open(os.path.join(output_annot_path, f"{image_counter}.xml"), 'w') as f:
