...
class YOLO:
    ...
    def init_model(self):
        ...
        self.model.load_weights("weights/yolo.h5")
...
if __name__ == '__main__':
    ...
    test_obj = YOLO("weights/yolo.h5")
    print("Creating Object...", time.time()-start_time)

    start_time = time.time()
    test_obj.init_model()
    print("Intializing Model...", time.time() - start_time)
    ...
    while True:
        ...
        test_obj.evaluate_frame(frame)
        cv2.putText(frame, f"FPS: {1/(time.time() - start): .1f}", (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA) 
